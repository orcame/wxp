...
import com.github.binarywang.demo.wxpay.ScanQRCodeRequest;
import com.github.binarywang.demo.wxpay.ScanQRCodeResponse;
...
public class WxPayController implements WxPayService {
	...
	public ScanQRCodeResponse scanQRCodeCallback(@RequestBody ScanQRCodeRequest info, HttpServletRequest request) {
		String productId = info.getProductId();
		String openId = info.getOpenid();
		String ip = request.getRemoteAddr();

		// TODO: Generate internal order and save in database.
		String orderId = "sj" + String.valueOf(System.currentTimeMillis());
		WxPayUnifiedOrderRequest order = new WxPayUnifiedOrderRequest();
		order.setProductId(productId);
		order.setOpenid(openId);
		order.setOutTradeNo(orderId);
		order.setBody("test product");
		...
		try {
			...
			WxPayUnifiedOrderResult result = this.wxService.unifiedOrder(order);
			response.setPrepayId(result.getPrepayId());
			
			// TODO: update the prepay id into the internal order.
		} catch (WxPayException ex) {
			response.setResultCode("FAIL");
			response.setReturnCode("FAIL");
			response.setErrCodeDes(ex.getMessage());
			response.setReturnMsg(ex.getMessage());
		}
		
		response.checkAndSign(this.getConfig(), true);
		return response;
	}
	...
	/**
	 * 扫码支付回调地址，根据productid和openid生成订单， 调用统一订单接口，获取prepay_id并返回给微信端
	 * 
	 */
	@Override
